...
const ResultFindProfile = ({ navigation, route }) => {
  ...
  const handleAddFriend = async () => {
    try {
      ...
      await http
        .post(`/v1/users/profile/friends/by-phone`, body, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
        .then(async (res) => {
          ...
          const data = res.data;
          Alert.alert("Gửi kết bạn thành công", data);
          setIsFriend(true);
        })
        .catch((error) => {
          Alert.alert("Error in request:", error);
        });
    } catch (error) {
  ...
  return (
    <ScrollView style={[styles.container, { marginTop: 40 }]}>
      ...
      <TouchableOpacity
        style={[styles.Button, styles.FisrtRightButton]}
      ...
      <TouchableOpacity
        style={[styles.Button, styles.SecondRightButton]}
        onPress={() => setMoreInfor(true)}
      >
        <Image
          ...
          <View>
            ...
            <View style={styles.avatarContainer2}>
              <Avatar
                ...
                rounded={true}
                source={
                  thumbnailAvatar ? { uri: thumbnailAvatar } : defaultAvatar
                }
              />
              <Text
                style={{
